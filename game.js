...
var openBoundary = function (row, col) {
    ...
    // get the cell element belonging to that row and column
    if(row<0 || row>=GAME.rows ||col<0 || col>=GAME.cols || GAME.Cells[row][col].classList.contains("open"))
        return ;
    var cell = GAME.Cells[row][col];
...
var endGame = function () {
    ...
    for (var row = 0; row < GAME.rows; row++) {
        for (var col = 0; col < GAME.cols; col++) {
            ...
            //openCell(row,col);
            if(GAME.layout[row][col]==-1 && cell.getAttribute("marked")=="false")
                openCell(row,col);
            ...
            /*else if(cell.getAttribute("marked")==true && GAME.layout[row][col]==-1){
...
var updateHallOfFame = function(time, outcome) {
    ...
    if(outcome){
        ...
        ti.innerText=getFormattedTime(time);
        var id=document.getElementById(toonid);
        id.classList.remove("icon-emo-sleep");
        id.classList.add("icon-emo-happy");
    }
}
...
var handleLeftClick = function (row, col) {
    ...
    if (validMove(row, col)) { // if user clicked on a closed cell
        if (GAME.layout[row][col] == -1) { // if it is a bomb cell
            ...
            GAME.Cells[row][col].setAttribute("class","blast");
...
var toggleMarkCell = function (row, col, marked) {
    ...
    if (validMove(row, col)) {
        ...
        if (marked) {
            ...
            GAME.flaggedCells--;
            GAME.updatedBombs++;
            updateBombsRemaining(GAME.updatedBombs);
        } else {
            ...
            a.setAttribute("class","red icon-golf");
            cell.appendChild(a);
            GAME.flaggedCells++;
            GAME.updatedBombs--;
            updateBombsRemaining(GAME.updatedBombs);
...
var handleMouseEvents = function (mouseButton, row, col, marked) {
    ...
    if(GAME.openCellCount+GAME.flaggedCells==GAME.rows*GAME.cols && GAME.flaggedCells==GAME.bombs)
        gameWon();
    //       call gameWon() method
}
...
var setBoardParams = function (level) {
    ...
    if (level == 'easy') {
        ...
        GAME.bombs = 10;
        GAME.updatedBombs=10;
    } else if (level == 'medium') {
        GAME.rows = GAME.cols = 16;
        GAME.bombs = 40;
        GAME.updatedBombs = 40;
    } else {
        GAME.rows = 16; GAME.cols = 30;
        GAME.bombs = 99;
        GAME.updatedBombs = 99;
    }
}
...
var setBombs = function (row, col) {
    ...
    for (var count = 0; count < GAME.bombs; count++) {
        ...
        var ranrow=randomInt(0,GAME.rows);
        var rancol=randomInt(0,GAME.cols);
        while(!((row-1>ranrow || row+1<ranrow) && (col-1>rancol || col+1<rancol) && GAME.layout[ranrow][rancol]!=-1)){
            ranrow=randomInt(row,GAME.rows);
            rancol=randomInt(row,GAME.cols);
        }
        GAME.layout[ranrow][rancol]=-1;
        //alert(ranrow+" "+rancol);
        updateNeighborCounts(ranrow,rancol);
...
var getFormattedTime = function(timeInSeconds) {
    ...
    var q=timeInSeconds/3600;
    var r=timeInSeconds%3600;
    var hrs=format(Math.floor(q),2);
    var min=format(Math.floor(r/60),2);
    var sec=format(Math.floor(r%60),2);
    return hrs+':'+min+':'+sec;
}
...
var startTimer = function () {
    ...
    }, 1000); // can you figure why 700 milliseconds when we want to udate clock every second?
}
...
var createGame = function (level) {
    stopTimer();
    document.getElementById("timer").innerText=getFormattedTime(0);
